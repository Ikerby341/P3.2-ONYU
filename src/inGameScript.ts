...
let hoveredButton2: HTMLButtonElement | null = null;
...
let activeCircle: ((e: TouchEvent) => void) | null = null;
    ...
    if (song) {
        ...
        try {
            ...
            if (raw) {
                ...
                if (typeof s.key === 'string') keyClickCircle = s.key;
            }
        } catch (err) {
            // ignore
        }
...
function començar(diff: string, gameArea: HTMLElement) {
    ...
    function spawnCircle() {
        ...
        function showFloatingText(text: string, color = 'white') {
        ...
        function handleHit(score: number) {
            if (clicked) return;
            clicked = true;
            if (score > 0) {
        ...
        function handleTouchStart(e: TouchEvent) {
            ...
            let score = 0;
            if (fraction >= 0.95) score = 100;
            else if (fraction >= 0.5) score = 50;
            else if (fraction >= 0.25) score = 25;
            else score = 0;
            handleHit(score);
        }
    ...
    function showVolumeSettings(pauseBox: HTMLElement) {
        ...
        slider.addEventListener('input', (e) => {
            ...
            // persist
            try { const cur = JSON.parse(localStorage.getItem('osu_settings')||'{}'); cur.volume = audio.volume; localStorage.setItem('osu_settings', JSON.stringify(cur)); } catch(e) {}
        });
        ...
        keyInput.addEventListener('keydown', (e) => {
            ...
            if (key.length === 1 || ['enter', ' ', 'arrowup', 'arrowdown', 'arrowleft', 'arrowright', 'shift', 'control', 'alt'].includes(key)) {
                ...
                // persist
                try { const cur = JSON.parse(localStorage.getItem('osu_settings')||'{}'); cur.key = keyClickCircle; localStorage.setItem('osu_settings', JSON.stringify(cur)); } catch(e) {}
            }
        });
        ...
        backBtn.addEventListener('mousedown', () => {
            ...
            pauseBox.appendChild(createButton('Reanudar', () => {
                ...
                (pauseBox.parentElement as HTMLElement).remove();
            }));
            pauseBox.appendChild(createButton('Ajustes', () => {
                showVolumeSettings(pauseBox);
            }));
